...
interface Manifest { schema_version: number; identity: CandidateIdentity; status: string; captures: { file: string; sha256: string; extension_version: string; commit: string; captured_at: string }[]; }
const digest = (file: string) => createHash("sha256").update(fs.readFileSync(file)).digest("hex");
export function createCandidateManifest(root: string, identity: CandidateIdentity): Manifest {
  return { schema_version: 1, identity, status: "awaiting_independent_visual_review", captures: fs.readdirSync(root).filter(n => /\.(png|mp4|gif)$/u.test(n)).sort().map(file => ({ file, sha256: digest(path.join(root, file)), extension_version: identity.extension_version, commit: identity.extension_commit, captured_at: fs.statSync(path.join(root, file)).mtime.toISOString() })) };
}
export function validateCandidateManifest(root: string, manifest: Manifest, expected: CandidateIdentity): void {
  ...
  for (const capture of manifest.captures) {
    if (capture.extension_version !== expected.extension_version || capture.commit !== expected.extension_commit) throw new Error("Capture entry identity mismatch");
